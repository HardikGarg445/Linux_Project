...
# Entry fields 
 ...
 #  Used to Place input fields in the grid
...
#Used this  Frame for the results table
...
# Color tags for open/closed ports
        ...
        self.tree.tag_configure('closed', background=self.colors["Light"]["output_bg"]) # Default for closed , green
...
# Pack buttons side by side
    ...
    def toggle_scan(self):
        if self.scanning:   # If already scanning, stop it
            self.stop_event.set()    #  Gives Signal the thread to stop
...
# Start scanning in a new thread
    ...
    def _scan_tcp(self, ip, port, timeout):
        ...
        self.tree.insert('', 'end', values=(port, 'TCP', 'TCP', status), tags=(tag,))   # adding to the table 
        self.scan_results.append({'port': port, 'type': 'TCP', 'status': status})    # Store result
    ...
    def _scan_udp(self, ip, port, timeout):
        try:
            ...
            s.close()
        tag = 'open' if status == "Open" else 'closed'
        self.tree.insert('', 'end', values=(port, 'UDP', 'UDP', status), tags=(tag,))    # Add to tabl
        self.scan_results.append({'port': port, 'type': 'UDP', 'status': status})    # Store result
