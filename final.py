...
class PortScannerApp:
    def __init__(self, root):
        ...
        self.current_theme = "Light"  # Set to Light theme initially
        ...
        self.colors = {
            ...
            "Light": {
                ...
                "dark_blue": "#15375C",
                "border": "#EAE8E8",
                "output_bg": "#FFFFFF",  # Fix: Set output_bg to white in Light theme
                "text": "#000000"
            }
        }
    ...
    def init_ui(self):
        ...
        self.percentage_text = self.circle_canvas.create_text(90, 70, text="0%", font=("Arial", 30, "bold"), fill="#FFFFFF")
        ...
        labels = ["Target Host/IP :", "Start Port :", "End Port :", "Scan Type :", "Time out (s) :"]

        for idx, label in enumerate(labels):
            ctk.CTkLabel(self.input_frame, text=label, font=self.custom_font).grid(row=idx, column=0, padx=10, pady=10, sticky="w")
        ...
        self.start_button = ctk.CTkButton(button_frame, text="Start Scan", command=self.start_scan, width=150, height=40, font=self.custom_font)
        self.start_button.pack(side="left", padx=10)

        self.save_button = ctk.CTkButton(button_frame, text="Save Results", command=self.save_results, width=150, height=40, font=self.custom_font)
        self.save_button.pack(side="left", padx=10)

        self.toggle_button = ctk.CTkButton(button_frame, text="Toggle Theme", command=self.toggle_theme, width=150, height=40, font=self.custom_font)
    ...
    def update_theme_colors(self):
        ...
        # Update Treeview background color for both themes
        style = ttk.Style()
        style.configure("Treeview", background=theme_colors["output_bg"], foreground=theme_colors["text"], fieldbackground=theme_colors["output_bg"])
        style.configure("Treeview.Heading", background=theme_colors["upper_bg"], foreground=theme_colors["text"])

        button_frame = self.toggle_button.master
        button_frame.configure(fg_color=theme_colors["lower_bg"])

        # Ensure buttons have white text on dark blue background
        self.start_button.configure(fg_color=theme_colors["dark_blue"], text_color="#FFFFFF")
        self.save_button.configure(fg_color=theme_colors["dark_blue"], text_color="#FFFFFF")
        self.toggle_button.configure(fg_color=theme_colors["dark_blue"], text_color="#FFFFFF")

        # Ensure entry fields have correct colors
        for entry in [self.entry_host, self.entry_start_port, self.entry_end_port, self.entry_timeout]:
            entry.configure(fg_color=theme_colors["upper_bg"], text_color=theme_colors["text"], border_color=theme_colors["border"])

        # Ensure scan type dropdown uses white text on dark blue
        self.scan_type.configure(
            fg_color=theme_colors["dark_blue"],
            text_color="#FFFFFF",  # White text on dark blue
            button_color=theme_colors["dark_blue"],
            dropdown_fg_color=theme_colors["dark_blue"],
            dropdown_text_color="#FFFFFF"  # Optional: dropdown items in white
        )
    ...
    def scan_ports(self, host, start_port, end_port, timeout):
        ...
        for i, port in enumerate(range(start_port, end_port + 1), 1):
            ...
            self.update_progress((i / total_ports) * 100)
    ...
    def scan_tcp(self, ip, port, timeout):
        try:
            ...
            tag = "open" if result == 0 else "closed"
            sock.close()
        except Exception:
            status, tag = "Error", "closed"

        item = self.tree.insert('', 'end', values=(port, 'TCP', 'TCP', status))
